...
describe("World", () => {
    ...
    describe("config.regions", () => {
        it("must set the list of possible region types", () => {
            ...
            expect(world.regionTypes).toEqual(["cave", "dungeon"]);
        });
    ...
    describe("config.chooseRegion", () => {
        it("must determine the type of region for a given position", () => {
            ...
            let chooseRegion = jest.fn(({ position, regionTypes }) =>
                position.x + position.y === 0 ? regionTypes[0] : regionTypes[1]
            );
            let world = World.create({
                regions,
                chooseRegion,
            ...
            Object.values(Direction.NEIGHBORS).forEach((dir) => {
                ...
                expect(world.getRegion(dir)).toEqual(expect.objectContaining({ type }));
            });
        });
